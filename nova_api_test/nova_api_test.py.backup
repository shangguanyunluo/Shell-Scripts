...
def create_vm(image_name=image_name, vm_name=vm_name, flavorname=flavor_name,
    ...
    server = find_vm_from_name(vm_name)
    if server is not None:
        print("There is already a vm named:%s" % vm_name)
        exit(1)

    flavor = openstack_utils.get_flavor_by_name(flavorname)
    image = openstack_utils.get_image_by_name(name=image_name)

    _nics = list()
    nets = neutronclient.list_networks(name=network_name)
    for _k in nets:
        for _v in nets[_k]:
    ...
    vm = server_client.create(name=vm_name, image=image.id,
                              flavor=flavor.id, nics=_nics,
                              key_name=key_name,
                              security_groups=security_groups)
    if not wait_for_vm_in_status(vm, status='ACTIVE'):
        raise Exception("Failed to build VM: %s" % vm)
